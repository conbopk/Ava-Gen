...
export async function getPresignedUrl(
    fileName: string,
    fileType: string,
    purpose: | "ptvPhoto" | "ptvAudio" | "ttsVoiceClone" | "videoTranslationSource" | "cvaVideo" | "cvaAudio",
) {
...
export async function photoToVideo(generateRequest: GenerateRequest) {
  ...
  console.log(db)
  console.log(db.photoToVideoGeneration)
  const photoToVideo = await db.photoToVideoGeneration.create({
    data: {
      name: generateRequest.script ? generateRequest.script.slice(0, 32) : new Date().toISOString(),
...
export interface ChangeVideoAudio {
  sourceVideoS3Key: string;
  newAudioS3Key: string;
  videoName: string;
}

export async function changeVideoAudio(request: ChangeVideoAudio) {
  const session = await auth.api.getSession({
    headers: await headers(),
  });
