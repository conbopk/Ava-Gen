...
import {Download, Image, Info, Languages, Play, RefreshCw, Video} from "lucide-react";
...
export function ClientHome({recentCreations}: {recentCreations: Creation[]}) {
  ...
  useEffect(() => {
    if (searchParams.get("purchase_success") === "true") {
      toast.success(
          "Purchase successfully. Your credits will be available shortly. Please refresh the page to view your updated balance.",
          { duration: 10000 },
      );
      ...
      <div className='p-8'>
        ...
        <div className='mb-12 flex flex-wrap gap-4'>
          {features.map((feature) => (
              ...
              >
                <div className={`flex items-center justify-center rounded-lg p-3 ${feature.color}`}>
                ...
                <span className='pointer-events-none absolute inset-0 rounded-lg border border-transparent opacity-0 shadow transition-all duration-300 group-hover:border-gray-200 group-hover:opacity-100 group-hover:shadow-md'/>
              </div>
          ))}
        </div>
                        ...
                        {item.status === "completed" && item.videoUrl ? (
                            <>
                              ...
                              <button
                                ...
                                className='absolute top-2 right-2 z-10 flex h-7 w-7 items-center justify-center rounded-full bg-black/60 text-white cursor-pointer opacity-0 transition-opacity  group-hover:opacity-100'
                              >
                        ...
                        {playingVideoId !== item.id && (
                            <>
                              ...
                              {durations[item.id] !== undefined && (
                                  <span className='bg-opacity-60 absolute right-2 bottom-2 bg-gray-800 px-2 py-0.5 text-xs text-white'>
        ...
        <VideoTranslationModal open={translateModalOpen} onOpenChange={setTranslateModalOpen}/>
        <ChangeVideoAudioModal open={changeVideoAudioOpen} onOpenChange={setChangeVideoAudioOpen}/>
      </div>
  );
}