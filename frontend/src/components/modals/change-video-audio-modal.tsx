...
import {ArrowLeft, Loader2, Pause, Play, Trash2, UploadCloud} from "lucide-react";
import AudioUploadModal from "~/components/audio-upload-modal";
import {changeVideoAudio, getPresignedUrl} from "~/actions/generations";


export function ChangeVideoAudioModal({open, onOpenChange}: {
  open: boolean,
  ...
  const handeGenerate = async () => {
    ...
    toast.success("Translation job queued! You will be notified upon completion.");
    setLoading(false);
    onOpenChange(false);
  };
      ...
      <>
          ...
          <DialogContent className='h-fit max-h-[95%] w-full min-w-[95%] overflow-y-auto lg:w-full lg:max-w-5xl lg:min-w-fit'>
            <DialogHeader>
              ...
              <DialogDescription className='mt-2 text-base'>
                Replace the audio of your video with a new track. The video can be up to{" "}
        ...
        <AudioUploadModal
          ...
          onAudioRecorded={(audioBlob: Blob) => {
            ...
            const file = audioBlob instanceof File ? audioBlob : new File([audioBlob], "new_record_" + new Date().toLocaleString().replace(/[\s:/,]/g, "_") + ".wav", { type: audioBlob.type });
