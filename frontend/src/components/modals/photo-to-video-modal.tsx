...
import {AudioWaveform, Loader2, Pause, Play, Ratio, Trash2, UploadCloud} from "lucide-react";
...
const samplePhotos = [
  {
    ...
    url: "https://public-hey-gen-clone-bk.s3.us-east-1.amazonaws.com/samples/photos/0001.jpg"
  },
  {
    s3Key: "samples/photos/0003.jpg",
    url: "https://public-hey-gen-clone-bk.s3.us-east-1.amazonaws.com/samples/photos/0003.jpg"
  },
  {
    s3Key: "samples/photos/0004.jpg",
    url: "https://public-hey-gen-clone-bk.s3.us-east-1.amazonaws.com/samples/photos/0004.jpg"
  },
];

export function PhotoToVideoModal({open, onOpenChange}:{
  open: boolean;
  ...
  const [selectedAudioName, setSelectedAudioName] = useState<string>("new_recording.wav");
  ...
  const handleGenerateVideo = async () => {
    ...
    if (selectedPhotoUrl) {
      ...
      if (!experimentalModel && croppedAreaPixels) {
        const croppedImage = await getCroppedImg(selectedPhotoUrl, croppedAreaPixels);
        if (croppedImage) {
    ...
    if (selectedAudioUrl && selectedAudioName) {
      ...
      const audioFile = new File([audioBlob], selectedAudioName, {type: audioBlob.type});

      const { url, key } = await getPresignedUrl(audioFile.name, audioFile.type, "ptvAudio");
    ...
    if (customVoiceFile) {
      const {url, key} = await getPresignedUrl(customVoiceFile.name, customVoiceFile.type, "ttsVoiceClone",);
    ...
    toast.success("Photo to video job queued! You will be ale to see the status on the dashboard.",)
        ...
        <DialogContent className='h-fit max-h-[95%] w-full min-w-[95%] overflow-y-auto lg:w-full lg:max-w-5xl lg:min-w-fit'>
          <div className='flex flex-col gap-0'>
            <DialogHeader>
              ...
              <DialogDescription>
                Turn a single photo and script into a high-quality avatar video using an AI portrait model.
              </DialogDescription>
            </DialogHeader>
              ...
              <div className='flex flex-1 flex-col gap-4'>
                <div>
                  <div className='relative'>
                    ...
                    {/*Container at the bottom or textarea*/}
                        ...
                        {!selectedAudioUrl &&
                            (customVoiceFile ? (
                                <div className='flex items-center gap-2'>
                                  <button
                                    className='flex items-center cursor-pointer gap-1 text-xs font-medium text-gray-600 hover:text-purple-600'
                ...
                <div className='flex gap-6'>
                  ...
                  {experimentalModel && (
                      <div className='flex items-center gap-2'>
                        <Switch checked={enhancement} onCheckedChange={setEnhancement} />
                ...
                <div className='mt-2 flex flex-wrap gap-6'>
                  ...
                  <div className='flex w-full items-center justify-end'>
                    ...
                    <Button
                        ...
                        disabled={loading || !((selectedPhotoFile ?? selectedPhotoUrl) && (script.trim().length > 0 || selectedAudioUrl))}
                    >
                      {loading ? (
          ...
          <AudioUploadModal
            ...
            onAudioRecorded={(audioBlob: Blob) => {
              ...
              let name = "new_recording_" + new Date().toLocaleString().replace(/[\s:/]/g, "_") + ".wav"
