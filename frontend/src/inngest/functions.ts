...
export const photoToVideo = inngest.createFunction(
    {
      ...
      onFailure: async ({ event, error }) => {
        await db.photoToVideoGeneration.update({
          where: {
            id: (event?.data?.event.data as { photoToVideoId: string }).photoToVideoId,
          },
          data: {
            status: "failed",
          },
    ...
    async ({ event, step }) => {
      ...
      if (photoToVideo.user.credits > 0) {
        ...
        if (photoToVideo.experimentalModel) {
          const [photoUrl, audioUrl] = await step.run("create-presigned-urls", async () => {
            const photo = await getPresignedUrl(photoToVideo.photoS3Key!);
            const audio = await getPresignedUrl(photoToVideo.drivingAudioS3Key!);
            return [photo, audio];
          });

          if (!photoUrl || !audioUrl)
            throw new Error("Missing photo or audio URL: " + photoUrl + ", " + audioUrl);
...
export const translateVideo = inngest.createFunction(
    {
      ...
      onFailure: async ({ event, error }) => {
        await db.videoTranslationGeneration.update({
          where: {
            id: (event?.data?.event.data as { videoTranslationId: string }).videoTranslationId,
          },
          data: {
            status: "failed",
          },
    ...
    async ({ event, step }) => {
      ...
      if (videoTranslation.user.credits > 0) {
        ...
        const videoUrl = await step.run("create-presigned-urls", async () => {
          return await getPresignedUrl(videoTranslation.sourceVideoS3Key!)
        });
        ...
        await step.run("deduct-credits", async () => {
          return await db.user.update({
            where: { id: videoTranslation.user.id },
            data: {
              credits: { decrement: 1 },
            },
          });
...
export const changeVideoAudio = inngest.createFunction(
    {
      ...
      onFailure: async ({ event, error }) => {
        await db.changeVideoAudioGeneration.update({
          where: {
            id: (event?.data?.event.data as { changeVideoAudioId: string }).changeVideoAudioId,
          },
          data: {
            status: "failed",
          },
    ...
    async ({ event, step }) => {
      ...
      if (generation.user.credits > 0) {
        ...
        const [videoUrl, audioUrl] = await step.run("create-presigned-urls", async () => {
        ...
        if (!videoUrl || !audioUrl)
          throw new Error("Missing video or audio URL: " + videoUrl + ", " + audioUrl);
