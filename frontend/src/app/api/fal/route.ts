...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!generation) {
      console.log(`No generation record found for requestId: ${request_id} - likely old webhook retry`);
      return NextResponse.json({ message: `No generation record found for requestId: ${request_id}` });
    }

    // Check if already processed
    if (generation.status === "completed" || generation.status === "failed") {
      console.log(`Generation ${request_id} already processed with status: ${generation.status}`);
      return NextResponse.json({ message: `Generation already processed with status: ${generation.status}` });
    }
    ...
    if (status === "OK") {
      ...
      if (!videoUrl) {
        console.log("No video URL found in payload:", payload);
      ...
      console.log(`Processing successful ${generationType} generation for ${request_id}, video URL: ${videoUrl}`);

      try {
        // Import the video from FAL to your S3 bucket
        const { s3Key } = await importVideoToS3(videoUrl);

        // Update the correct table based on generation type
        await updateGenerationStatus(generationType, generation.id, "completed", s3Key);

        return NextResponse.json({ message: `Webhook processed successfully for FAL ${generationType} generation` });
      } catch (importError) {
    ...
    } else if ( status === "failed" || status === "error" || status === "ERROR") {
      // Update the correct table based on generation type
      await updateGenerationStatus(generationType, generation.id, "failed");

      const errorMessage = payload.error ?? payload.payload?.detail ?? "Unknown error";
    ...
    console.log("FAL Webhook parse errors:", parseError);
...
async function importVideoToS3(videoUrl: string) {
