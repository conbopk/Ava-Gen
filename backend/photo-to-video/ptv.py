...
class PortraitAvatarServer:
    ...
    def generate_video(self, request: PortraitAvatarRequest) -> PortraitAvatarResponse:
        print(f"Received request to generate portrait avatar: {request.transcript} {request.photo_s3_key} {request.audio_s3_key}")
        ...
        try:
            ...
            ffmpeg_command = [
                "ffmpeg", "-i", generated_video_file, "-i", audio_path, "-c:v", "copy", "-c:a", "aac", "-shortest", final_video_path
            ]
...
def main():
    ...
    request = PortraitAvatarRequest(
        transcript="",
        photo_s3_key="samples/photos/0003.jpg",
